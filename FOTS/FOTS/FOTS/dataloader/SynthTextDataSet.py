...
import imgaug.augmentables.polys as ia_polys # Bounding boxes
import imgaug.augmentables.segmaps as ia_segmaps # Creating score map
from ..utils.util import str_label_converter # Converting strings to labels
from .datautils import check_and_validate_polys, normalize_image # Checking if bboxes are valid and normalizing image
from . import utils as data_utils # Get score geometry 
import scipy.io as sio # Load matlab file with labels

class SynthTextDataset(Dataset):
    def __init__(
        self, data_root, scale: float = 0.25, size: int = 640, transform=None, vis=False
    ):
    ...
    def visualize(
        ...
        polygons_on_image = ia_polys.PolygonsOnImage(
            polygons=polygon_list, shape=image.shape
        )
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            #print(f"Accessing path: {(self.dataRoot / image_path).as_posix()}")

            imgPath = pathlib.Path(self.dataRoot/image_path)

            if not imgPath.is_file():
                return self.__getitem__(np.random.randint(0, len(self)))

            im = cv2.imread((self.dataRoot / image_path).as_posix())

            image_path = pathlib.Path(image_path)
            word_b_boxes = (
                np.expand_dims(word_b_boxes, axis=2)
                if (word_b_boxes.ndim == 2)
                else word_b_boxes
            )
            ...
            if num_of_words == len(transcripts):
                ...
                if self.transform:
                    while True and (max_tries != 0):
                        ...
                        #print(f"Tramsformed Image Dimensions: {transformed_im.shape}\n")
                        valid_text_polys = [
                            polygon
                ...
                #print(f"Transformed? : {np.sum(im)==np.sum(transformed_im)}\n")
