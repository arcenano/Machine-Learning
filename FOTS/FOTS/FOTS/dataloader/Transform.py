import imgaug.augmenters as iaa  # Image Operation Functions
from imgaug.augmentables.polys import Polygon, PolygonsOnImage # Bounding Boxes handling
...
class Transform:
    ...
    def __call__(
        self, *args, **kwargs
    ) -> typing.Tuple[np.ndarray, typing.List[Polygon]]:

        if self.is_training:
            resize = iaa.Resize(
                size=dict(longer_side=self.long_sizes, width="keep-aspect-ratio")
            )
            rotate = iaa.Rotate(rotate=self.angles, fit_output=True)
            resize_height = iaa.Resize(
                size=dict(height=self.height_ratios, width="keep")
            )
            crop = iaa.CropToFixedSize(
                width=self.cropped_size[0], height=self.cropped_size[1],
            )
            fix_resize = iaa.Resize(size=self.output_size)
            blur = iaa.GaussianBlur()
            blur = iaa.Sometimes(p=self.blur_prob, then_list=blur)
            color_jitter = iaa.MultiplyBrightness()
            color_jitter = iaa.Sometimes(
                p=self.color_jitter_prob, then_list=color_jitter
            )

            augs = [crop, resize, rotate, resize_height, crop, fix_resize, blur, color_jitter]
