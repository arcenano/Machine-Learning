...
import glob as gb # Opening files
import numpy as np # Handling images
import cv2 # Reading and handling images
import csv # Parsing labels
from shapely.geometry import Polygon # Extracting polygon areas
...
def collate_fn(batch):
    imagePaths, img, score_map, geo_map, training_mask, transcripts, boxes, rois = zip(
        *batch
    )
...
def normalize_image(
    image: np.ndarray, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]
):
    image = image / 255
