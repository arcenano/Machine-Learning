...
from ..dataloader import datautils
from . import lanms
...
class Toolbox:
    ...
    def restore_rectangle_rbox(origin, geometry):
        ...
        if origin_0.shape[0] > 0:
            ...
            rotate_matrix_x = np.array([np.cos(angle_0), np.sin(angle_0)]).transpose(
                (1, 0)
            )
            rotate_matrix_x = (
    ...
    @staticmethod
    def detect(
        score_map, geo_map, timer, score_map_thresh=0.8, box_thresh=0.1, nms_thres=0.2
    ):
        """1e-5
    ...
    def predict(im_fn, model, with_img=False, output_dir=None, with_gpu=False):
        ...
        im_resized = datautils.normalize_image(im_resized)
