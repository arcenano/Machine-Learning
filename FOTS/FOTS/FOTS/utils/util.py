...
def show_box(image_name, image, box, transcirpt, isFeaturemap=False):
    ...
    img = cv2.putText(
        img, transcirpt, (origin[0], origin[1] - 10), font, 0.5, (255, 255, 255)
    )
...
def visualize(image_path: str, boxes: numpy.ndarray, transrcipts: typing.List[str]):
    ...
    for box, transcript in zip(boxes, transrcipts):
        ...
        font = cv2.FONT_HERSHEY_PLAIN
        img = cv2.putText(
            img, transcript, (origin[0], origin[1] - 10), font, 0.5, (255, 255, 255)
        )
...
class StringLabelConverter(object):
    ...
    def __init__(
        self, alphabet, ignore_case=False, max_length=50, raise_over_length=True
    ):
    ...
    def encode(self, text):
        ...
        if isinstance(text, str):
            text = [
                self.dict.get(
                    char.lower() if self._ignore_case else char, self.dict["<other>"]
                )
                for char in text
            ]
            length = [len(text)]
            ...
            for s in text:
                ...
                if len(text) > self.max_length:
                    if self.raise_over_length:
                        raise ValueError(
                            "{} is over length {}".format(text, self.max_length)
                        )
                    else:
